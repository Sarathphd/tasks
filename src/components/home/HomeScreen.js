...
const HomeScreen = ({ navigation }) => {
    ...
    const renderItem = ({ item, i, index, move, moveEnd, isActive }) => (
        <TouchableOpacity onPress={() => detailScreen(item)} onLongPress={move}
        onPressOut={moveEnd}>
            ...
            />
                ...
                {filterdData && filterdData.length ? (
                    <DraggableFlatList
                        ...
                        scrollPercent={5}
                        onMoveEnd={({ filterdData }) => setfilleredData({ filterdData })}
                    />
